...
只有循环最终观察到一次 `fd>=3` 时，它才证明 0、1、2 当时都非空；仍不保证三个槽分别具有理想的读写权限，正常继承自 `init` 才提供完整语义。若 `open("console")` 在填满空槽前返回 `-1`，`while` 会静默结束，shell 不诊断也不复查，后续 prompt/read/write 可能失败或落到错误的继承对象。

## 6. shell 的顶层读取循环

`getcmd()` 向 fd 2 写 `$ `，清空 100 字节静态缓冲区，再用 `gets()` 从 fd 0 读取一行。`gets()` 对首个 `read()` 的 EOF 和错误都只是停止并写入 NUL；两者都会留下空字符串，使 shell 退出并由 `init` 重启。当前接口无法把真实控制台 EOF 与 fd 0 不可读、坏 fd 等首读失败区分开。
...
| 类型 | 结构 | 语义 |
|---|---|---|
| `EXEC` | `struct execcmd` | 最多 9 个非空 `argv` 项（包含程序名） |
...
所有结构的第一个成员都是 `int type`，所以 `struct cmd *` 可以作为手写的 tagged union 使用。`runcmd()` 先检查 `type`，再转换为相应具体结构。构造器通过 `malloc()` 分配节点并整体清零，但都没有检查 `malloc()` 是否返回 0。分配失败后紧接着的 `memset(0, ...)` 会尝试写用户虚拟地址 0；当前 `_sh` 在该地址映射的是只读可执行 text，因此 store page fault 不能由 `vmfault()` 修复，该命令执行子进程会被杀死。
...
包装顺序可以从 parser 精确推出：每读到一个重定向，就用新 `REDIR` 包住当前 AST，所以命令行中最后一个重定向位于最外层、反而最先执行。例如 `echo x > a > b` 会先创建/截断 `b`，再关闭 fd 1 并创建/截断 `a`，最终输出写入 `a`；`b` 仍留下为空。若随后打开内层目标失败，已经发生的外层创建/截断也不会回滚。
...
| AST 节点的 `malloc()` 失败 | 构造器未检查空指针，随后写地址 0 触发 fault；解析位于 line runner，主 shell 等到该子进程退出后继续下一行 |
| 空行 | 顶层 shell 忽略，不派生进程 |
| fd 0 EOF 或首个 `read()` 错误 | `gets()` 都返回空字符串；shell 退出，`init` 随后重新启动它 |
