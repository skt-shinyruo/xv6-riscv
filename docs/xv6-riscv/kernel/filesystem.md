...
- `nlink` 是文件系统维护的持久链接计数。非目录 inode 上它等于指向 inode 的硬链接名字数；成功创建且仍在 root 可达树中的非根目录以父目录中的名字为基础计数 1，每个可达子目录的 `..` 再为它增加一次，而自己的 `.` 不计数。根目录由 `mkfs` 特别初始化为 1。因此目录的 nlink 不是所有 dirent 的字面计数；归零表示 inode 已没有受计数的命名/目录结构链接。崩溃前仍被 cwd 引用的空目录 orphan 也可能保留陈旧 `..`，但该项不再代表有效 parent edge 或 nlink 贡献。
...
`ref == 1` 的证明只保证 `iput()` 在仍持 `itable.lock` 时取得 inode sleeplock不会等待；它没有阻止释放表锁后的新 `iget(dev, inum)` 按相同 identity 把 `ref` 从 1 增到 2。普通文件失去最后一个目录项后通常没有新的路径来源，但未链接目录留下了一个正常 API 反例：若 P 的 cwd 是 `/a/b`，Q 依次 unlink `/a/b` 和 `/a`，仍存活的 `b/..` 会继续保存 `a` 的 inum。P 的相对 `namei("..")` 可经 `dirlookup()->iget(a)` 与 `a` 的最后一次 `iput()` 并发；`iget()` 在回收者释放 `itable.lock` 后增加引用，随后 `ilock(a)` 等待回收锁，最终从 dinode 读到 `type==0` 并 `panic("ilock: no type")`。

这个残留入口在回收完成后也没有 generation 保护：若旧 inum 仍 free，`ilock()` 同样 panic；若已复用成普通文件，`chdir()` 在 type 检查处返回 -1；若已复用成另一个目录，相对 `..` 甚至会进入无关目录。因而当前协议只证明回收者取得 sleeplock时不会阻塞，并未封堵 orphan 目录的陈旧 `..` 在回收期间或之后重新解析旧 inum；不能把“cache 槽暂时仍可取得引用”解释成受支持的并发 reopen 语义。
...
- `bn >= MAXFILE` 时直接 panic；合法逻辑块号的半开区间是 `[0, MAXFILE)`。
...
重复斜杠和尾随斜杠都被折叠，例如 `///a//b/` 按 `a`、`b` 两个元素处理。尾随 `/` 不携带“最终对象必须是目录”的语义：`namei("file/")` 可以返回普通文件，`open("file/", O_RDONLY)` 与 `unlink("file/")` 也按同一个 `file` 名处理。`.` 与 `..` 没有语法特判，只是目录中由创建逻辑维护的普通条目。
...
`namei(path)` 返回最终 inode；`nameiparent(path, name)` 返回父目录并给出最后一项。返回值都是“有引用、未锁”。没有任何路径元素时行为取决于起点：`namei("")` 直接返回 cwd，`namei("/")` 和 `namei("////")` 直接返回 root；而 `nameiparent()` 对空串或任意全斜杠路径都没有可返回的末元素，会释放起点引用并失败。失败条件不只限于单个 `/`。
...
5. 对良构镜像，`size` 范围内每个逻辑块应存在，当前运行时不把零指针实现为稀疏洞。内核不会完整验证该前提，`mkfs` 在根目录 size 恰好对齐时的向上调整也可制造 size 内空洞；损坏/异常镜像上的 `readi()` 甚至可能经 `bmap()` 尝试分配。
6. 对良构镜像，目录有效项的 inode 号非零且在范围内，名字按 14 字节比较，同一目录不得有两个同名有效项。运行时 `dirlink()` 会拒绝重复名，但 `mkfs` 对去前缀后的输入名不做全局去重，内核也不在挂载时扫描验证。
7. 对 root 可达的良构目录，除根目录外每个创建成功的目录有 `.` 指向自己、`..` 指向父目录；已提交 unlink 后仍被 cwd 引用的 recoverable orphan 目录是例外：它的 `.` 仍自指，`..` 只保证 inode 号在范围内且不等于自身，旧 parent 可以已释放或复用。
8. 系统调用路径禁止目录硬链接并拒绝 unlink 非空目录，因而不能通过正常 API 制造目录环；磁盘镜像中的目录图仍是被信任输入。
9. 可能释放或分配磁盘资源的路径必须在日志事务内。
10. 日志恢复必须先于 orphan 扫描。

这些良构条件的可执行定义、损坏传播和安全检查顺序见[文件系统一致性](../filesystem/filesystem-consistency.md)。
