...
两个整数是分两次复制的。如果第一个复制成功、第二个失败，内核资源会完整回滚，系统调用返回 `-1`；用户内存中第一个整数会保留，第二个若跨页还可能只写入有效页尾的 1 至 3 个字节。两个数都已失效，用户缓冲区不具备事务性，调用者必须丢弃整个数组。
...
这里仍有检查后入睡的取消窗口。pipe reader/writer 持 `pi->lock` 检查 killed 后才调用 `sleep()`，而 `kkill()` 只取 `p->lock`、不取 `pi->lock`；若 kill 先看到目标仍为 `RUNNING` 并置位，目标随后仍可发布 `SLEEPING`。真实的对端读写/close 或第二次 kill 会令它再次运行并观察标志，但第一次 kill 本身不保证立即解除这次尚未发布的阻塞。
...
当前 shell 没有检查两次 `dup()` 的返回值，但在这条已建立的正常拓扑中，它们并不依赖“还有额外空闲 fd”：child 先关闭目标 1 或 0，立即制造一个最低空槽，而作为源的 pipe endpoint 仍是有效 fd，所以 `fdalloc()` 必然能占用刚释放的槽。`NOFILE` 耗尽本身不能让这两次 dup 失败；若源端点已损坏、标准 fd 预设不成立或未来改变操作顺序，未检查返回值才会把错误拓扑继续交给 exec。

叶进程的 `exec()` 若失败，`user/sh.c:runcmd(EXEC)` 打印错误后会落到函数末尾 `exit(0)`，因此失败命令反而以状态 0 退出。退出仍会关闭继承的 pipe endpoint，使对端 EOF/破管道继续推进；pipeline 管理进程又忽略两次 `wait()` 的状态，所以当前 shell 不把这种 exec 失败传播为 pipeline 失败状态。
